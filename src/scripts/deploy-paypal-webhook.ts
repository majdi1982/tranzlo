// Tranzlo Appwrite Function Deployment Automator for PayPal Webhooks
// Run: npx tsx src/scripts/deploy-paypal-webhook.ts
//
...
const {
  ...
  PAYPAL_SANDBOX_WEBHOOK_ID: sandboxWebhookId,
...
const FUNCTION_ID = "paypal-webhook-handler";
const FUNCTION_NAME = "Tranzlo PayPal Webhook Handler";
...
async function main() {
  ...
  const tempDir = path.resolve(process.cwd(), "temp-paypal-webhook");
  if (!fs.existsSync(tempDir)) {
    fs.mkdirSync(tempDir);
  }

  // 1. Write index.js (copied from appwrite-function-paypal-webhook.js)
  const sourceCode = fs.readFileSync(
    path.resolve(process.cwd(), "src/scripts/appwrite-function-paypal-webhook.js"),
    "utf-8"
  );
  ...
  const packageJson = {
    ...
    version: "1.0.0",
    description: "Tranzlo PayPal Webhook Handler",
    main: "index.js",
    dependencies: {
  ...
  if (!funcExists) {
    ...
    // Update existing function settings/schedule
    try {
      ...
      console.log("   ✅ Function settings updated successfully!");
    } catch (updateErr: any) {
  ...
  const vars = {
    ...
    NEXT_PUBLIC_APPWRITE_ENDPOINT: endpoint!,
    NEXT_PUBLIC_PAYPAL_WEBHOOK_ID: paypalMode === "sandbox" ? sandboxWebhookId || "" : webhookId || "",
    NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PRO_MONTHLY: process.env.NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PRO_MONTHLY || "",
    NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PRO_ANNUAL: process.env.NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PRO_ANNUAL || "",
    NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PLUS_MONTHLY: process.env.NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PLUS_MONTHLY || "",
    NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PLUS_ANNUAL: process.env.NEXT_PUBLIC_PAYPAL_PLAN_TRANSLATOR_PLUS_ANNUAL || "",
    NEXT_PUBLIC_PAYPAL_PLAN_COMPANY_PRO_ANNUAL: process.env.NEXT_PUBLIC_PAYPAL_PLAN_COMPANY_PRO_ANNUAL || "",
    NEXT_PUBLIC_PAYPAL_PLAN_COMPANY_PLUS_ANNUAL: process.env.NEXT_PUBLIC_PAYPAL_PLAN_COMPANY_PLUS_ANNUAL || "",
  };
