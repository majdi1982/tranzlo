// Tranzlo Appwrite Function Automatic Deployer
// Run: npx tsx src/scripts/deploy-appwrite-function.ts
//
...
const FUNCTION_NAME = "RSS Auto-Publisher with Gemini AI";
...
async function main() {
  ...
  const tempDir = path.resolve(process.cwd(), "temp-appwrite-function");
  if (!fs.existsSync(tempDir)) {
    fs.mkdirSync(tempDir);
  }

  // 1. Write index.js (copied from appwrite-function-publish-rss.js)
  const sourceCode = fs.readFileSync(
    path.resolve(process.cwd(), "src/scripts/appwrite-function-publish-rss.js"),
    "utf-8"
  );
  ...
  const packageJson = {
    name: "rss-auto-publisher",
    version: "1.0.0",
    description: "Appwrite RSS Auto-Publisher with Gemini AI",
    main: "index.js",
    dependencies: {
  ...
  if (!funcExists) {
    try {
      await functions.create(
        ...
        300, // timeout (5 minutes, must be integer)
        true // enabled
      );
