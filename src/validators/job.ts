...
export const createJobSchema = z.object({
  ...
  testDuration: z.number().positive("Duration must be positive").max(48, "Test duration cannot exceed 48 hours (2 days)").optional(),
  testWordCount: z.number().positive("Word count must be positive").max(250, "Word count must be at most 250 words").optional(),
  maxTestApplicants: z.number().positive("Maximum test applicants must be positive").max(10, "Maximum 10 applicants allowed for test").optional(),
