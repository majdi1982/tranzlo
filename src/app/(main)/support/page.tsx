...
export default function SupportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.$id) {
      alert("⚠️ You must be logged in to submit a support ticket.");
      return;
    }

    if (message.length < 20) {
      alert("⚠️ Description must be at least 20 characters.");
      return;
    }
    ...
    try {
      ...
      // Clear form except name/email of active user
      setSubject("");
      setMessage("");
          ...
          <div className="lg:col-span-5">
            <div className="border border-border rounded-2xl p-6 md:p-8 bg-background/60 shadow-md space-y-6 backdrop-blur-md">
              ...
              {user ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <option value="billing">Payments & Subscriptions</option>
                      <option value="verification">Account Verification</option>
                      <option value="language_change">Language Change Request</option>
                    </select>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center p-6 border border-dashed border-border rounded-xl bg-accent/5 text-center space-y-4">
                  <AlertCircle className="h-8 w-8 text-muted-foreground opacity-60 animate-pulse" />
                  <div className="space-y-1">
                    <h4 className="font-bold text-sm">Login Required</h4>
                    <p className="text-[10px] text-muted-foreground">You must log in to your account to submit support tickets.</p>
                  </div>
                  <Button asChild size="sm" className="rounded-xl shadow-md bg-primary text-primary-foreground hover:bg-primary/95">
